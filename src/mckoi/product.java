...
public class product extends JInternalFrame {
	 ...
	 */
	public product() {
		setFrameIcon(new ImageIcon("D:\\PBO_UAS_1\\mckoi\\images\\png_tambahan\\Box-icon.png"));
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
		        ...
		        try {
		            ...
		            ResultSet rs = st.executeQuery("select Nama from supplier where id='"+idp+"'");
		            while (rs.next()) {
		...
		nmS.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
		        ...
		        try {
		            Connection konek = connect.getCon();
		            Statement st = konek.createStatement();
		            ResultSet rs = st.executeQuery("select id from supplier where Nama='"+nama+"'");
		            while (rs.next()) {
